...
class FPFH(PFH):
    ...
    def calcHistArray(self, pc, norm, indNeigh):
        ...
        for i in range(N):
            ...
            features = np.zeros((len(pc[i]), 3))
            for j in range(len(pc[i])):
                pi = pc[i]
                pj = pc[int(indNeigh[i][j])]
                if np.arccos(np.dot(norm[i], pj - pi)) <= np.arccos(np.dot(norm[j], pi - pj)):
                    ps = pi
                    pt = pj
                    ns = np.asarray(norm[i]).squeeze()
                    nt = np.asarray(norm[int(indNeigh[i][j])]).squeeze()
                else:
                    ps = pj
                    pt = pi
                    ns = np.asarray(norm[int(indNeigh[i][j])]).squeeze()
                    nt = np.asarray(norm[i]).squeeze()
        ...
        for i in range(N):
            k = len(indNeigh[i])
            for j in range(k):
                spfh_sum = histArray[int(indNeigh[i][j])]*(1/distList[i][j])
